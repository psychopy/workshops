...
expName = 'posnerHand'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/lpzjwp/OneDrive - The University of Nottingham/workshops/examples2019/builder/posnerHand/posnerHand_lastrun.py',
...
for thisTrial in trials:
    ...
    trialComponents = [fixation, cue, target, mouse]
    ...
    while continueRoutine:
        ...
        # *cue* updates
        if t >= 1.0 and cue.status == NOT_STARTED:
            # keep track of start time/frame for later
            cue.tStart = t
        ...
        # *target* updates
        if t >= 1.2 and target.status == NOT_STARTED:
            # keep track of start time/frame for later
            target.tStart = t
            target.frameNStart = frameN  # exact frame index
            target.setAutoDraw(True)
        # *mouse* updates
        if (1.2) and mouse.status == NOT_STARTED:
            # keep track of start time/frame for later
            mouse.tStart = t
